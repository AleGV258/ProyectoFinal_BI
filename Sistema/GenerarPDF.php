<?php
    ...
    function integracionPDF($NombreArchivo, $TipoArchivo, $CErrores, $Insertados, $Total, $Filtro, $Errores, $Correo, $Usuario)
    {
        ...
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 122);
        $pdf->Cell(165, 3, "Usuario:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 122);
        $pdf->Cell(165, 3, $Usuario, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 129);
        $pdf->Cell(165, 3, "Correo:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 129);
        $pdf->Cell(165, 3, $Correo, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 136);
        $pdf->Cell(165, 3, "Nombre del Archivo:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 136);
        $pdf->Cell(165, 3, $NombreArchivo, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 143);
        $pdf->Cell(165, 3, "Tipo de Archivo:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 143);
        $pdf->Cell(165, 3, $TipoArchivo, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 150);
        $pdf->Cell(165, 3, "Total de Datos del Archivo:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 150);
        $pdf->Cell(165, 3, $Total, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 157);
        $pdf->Cell(165, 3, "Datos Insertados en la Base de Datos:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 157);
        $pdf->Cell(165, 3, $Insertados, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 164);
        $pdf->Cell(165, 3, "Datos No Insertados en la Base de Datos:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 164);
        $pdf->Cell(165, 3, $CErrores, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 171);
        $pdf->Cell(165, 3, "Datos Insertados Correctamente ($PorcentajeInsertados %).", 0, 2, 'C');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 178);
        $pdf->Cell(165, 3, "Datos No Insertados ($PorcentajeErrores %).", 0, 2, 'C');

        $Posicion = 185;
        for ($i = 0; $i < count($Errores); $i++) {
            if ($Filtro[$i] != 'NO') {
                $pdf->SetXY(25, $Posicion);
                $Posicion = $Posicion + 7;
                $pdf->Cell(165, 3, "Filtro '$Filtro[$i]': $Errores[$i] Errores Encontrados.", 0, 2, 'C');
            }
        }
    ...
    function analisisPDF($tabla, $k, $j, $m, $a, $pronostico, $Correo, $Usuario)
    {
        ...
        $pdf->Cell(165, 3, $Correo, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 136);
        $pdf->Cell(165, 3, "Tabla Analizada:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 136);
        $pdf->Cell(165, 3, $tabla, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 143);
        $pdf->Cell(165, 3, "Valor de K:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 143);
        $pdf->Cell(165, 3, $k, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 150);
        $pdf->Cell(165, 3, "Valor de J:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 150);
        $pdf->Cell(165, 3, $j, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 157);
        $pdf->Cell(165, 3, "Valor de M:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 157);
        $pdf->Cell(165, 3, $m, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 164);
        $pdf->Cell(165, 3, "Valor de A (Alfa):", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 164);
        $pdf->Cell(165, 3, $a, 0, 2, 'R');
        $pdf->SetFont('Arial', 'B', 14); $pdf->SetXY(25, 171);
        $pdf->Cell(165, 3, "Valor de Pronostico:", 0, 2, 'L');
        $pdf->SetFont('Arial', '', 14); $pdf->SetXY(25, 171);
        $pdf->Cell(165, 3, $pronostico, 0, 2, 'R');
